...
- 不堆砌华丽词藻，宁可朴实直接。
- 【具体化硬规则·这一条最重要·用户反馈"清晰度不够"的主因】禁止抽象人格标签型句式。下面是必须避免和必须做到的对照：
  反例(不要这样写)：
    "你是一个有深度的人，外表平静内心丰富。"
    "你思维细腻、情感深邃，擅长在复杂情境中保持清醒。"
    "你对关系有极强的感知力，能看见别人看不到的层次。"
  这些句子套到任何人身上都说得通，等于没说，是用户反馈"清晰度不够"的主要来源。
  正例(必须这样写，每一段至少 1-2 个这种"具体场景锚定句")：
    "你跟朋友约时间，对方临时改三次，你不会当场抱怨，但回家会反复想他是不是其实不在乎。"
    "同事开会发飙时，你会先看大家的反应再决定是否发言；事后会私下问那个被批评的人有没有事。"
    "你买衣服可以反复纠结两小时，但下决定后基本不退；做事的节奏不是冲刺型，而是憋着一股劲一次性做完。"
  以上场景仅为"句式结构"示例，不要在你的输出里复用这些具体场景；要根据当前用户本命盘的实际配置(月亮星宿、上升、命主星状态、落陷与否、dasha 等)即兴生成贴合这个人的具体场景。
  写法上的硬规则：
  · 每段至少给 1-2 个"在 X 情境下你倾向于 Y"式的具体行为锚定，不是堆形容词。
  · 形容词只能在锚定句之后做一句收尾总结，不能单独成段也不能堆砌。
  · 如果某段你写不出 1-2 个具体行为锚定，宁可整段更短，也不要用空洞形容词凑字数。
  · 锚定句要"具体到能让用户回想起自己确实做过这种事"的程度，不是泛泛的人格描述。
